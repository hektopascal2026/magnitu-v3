...
- [ ] **2.1** Navigation: add **“Gemini”** tab next to **“Label”** (same patterns as existing templates / `main.py` routes).
- [ ] **2.2** **Target header**: show **active model profile** name (ties to Phase 3; can stub with current single profile first if needed).
- [ ] **2.3** **Batch scoring**: pull entries from **“Uncertain”** and/or **“New”** queues (reuse existing queue logic; document which queues).
- [ ] **2.4** For each entry: call Phase 1 pipeline → `predicted_label` + `reasoning_note`.
- [ ] **2.5** **Persist**: save to **`labels.json`** in the **established format** so **Train** / recipe pipeline applies **1.5× reasoning weight** correctly.
- [ ] **2.6** **Idempotency / safety**: define behavior when an entry is already labeled (skip, overwrite, or tag source=`synthetic`) — implement consistently.
- [ ] **2.7** Surface errors (rate limits, partial batch failure) in UI; allow retry of failed subset.

### Acceptance

- [ ] User can run a batch from the Gemini tab and see labels + reasoning persisted for training.
- [ ] `labels.json` remains compatible with existing train/recipe code paths.
